...
sealed class ButterfliesCritob : Critob
{
    internal ButterfliesCritob() : base(Enums.CreatureTemplateType.Butterflies)
    {
        Icon = new SimpleIcon("Kill_Standard_Lizard", Color.yellow);
        LoadedPerformanceCost = 50f;
        SandboxPerformanceCost = new(.5f, .5f);
        RegisterUnlock(KillScore.Configurable(9), Enums.SandboxUnlockID.Butterflies);
    }
    ...
    public override IEnumerable<string> WorldFileAliases() => ["Butterflies"];

    //public override ArtificialIntelligence? CreateRealizedAI(AbstractCreature acrit) => new FlyAI(acrit, acrit.world);
    public override ArtificialIntelligence? CreateRealizedAI(AbstractCreature acrit) => new ButterfliesAI(acrit, acrit.world);

    public override Creature CreateRealizedCreature(AbstractCreature acrit) => new Butterflies(acrit, acrit.world);

    //public override CreatureState CreateState(AbstractCreature acrit) => new ButterfliesState(acrit);
